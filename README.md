...
Built an end-to-end RAG-based document assistant that allows users to upload TXT, PDF, and DOCX files and ask questions grounded in document content. Implemented FastAPI backend, MongoDB metadata storage, Pinecone vector search, Gemini embeddings, Groq LLM generation, chat sessions, short-term memory, question rewriting, retrieval evaluation, Docker setup, and Streamlit frontend prototype.
